...
describe("CacheManager", () => {
  ...
  test("should get cache information", () => {
    ...
    expect(allCacheInfo.some((info) => info.name === "Info Test Cache")).toBe(
      true
    );
  });
  ...
  test("should update cache configuration", () => {
    ...
    const updatedNonExistent = cacheManager.updateCacheConfig(
      "nonExistentCache",
      {
        maxSize: 30,
      }
  ...
  test("should estimate memory usage", () => {
    ...
    testCache.set(
      "key2",
      "this is a much longer value that should take more memory"
    );
