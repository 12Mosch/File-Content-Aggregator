...
import { evaluateSearchExpression } from '../../src/electron/fileSearchService';
...
describe('Memory Leak Detection Tests', () => {
  ...
  test('should detect memory leaks in repeated search operations', async () => {
    ...
    const totalMemoryChange = (finalMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024;
    ...
    // Save results
    await saveTestResults('memory-leak-detection', { summary, iterations: results });
    ...
    const avgMemoryChange = memoryChanges.reduce((sum, change) => sum + change, 0) / memoryChanges.length;
    
    console.log(`Average memory change per iteration: ${avgMemoryChange.toFixed(2)} MB`);
    ...
    if (avgMemoryChange > MEMORY_LEAK_THRESHOLD) {
      console.warn(`Potential memory leak detected: Average memory increase of ${avgMemoryChange.toFixed(2)} MB per iteration`);
    } else {
  ...
  test('should detect memory leaks in word boundary cache', async () => {
    ...
    // Save results
    await saveTestResults('word-boundary-cache-memory-test', { summary, iterations: results });
