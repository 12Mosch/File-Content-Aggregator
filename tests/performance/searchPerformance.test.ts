  ...
  describe('Term Search Performance', () => {
    test('should measure performance of simple term search', async () => {
      ...
      for (const size of testSizes) {
        ...
        // Log results
        console.log(`Term search (${contentSizeMB.toFixed(2)} MB): ${executionTime.toFixed(2)} ms, Memory: ${memoryUsed.toFixed(2)} MB`);
    ...
    test('should measure performance of complex boolean queries', async () => {
      ...
      for (const size of testSizes) {
        ...
        for (const query of queries) {
          ...
          // Log results
          console.log(`Boolean query "${query}" (${contentSizeMB.toFixed(2)} MB): ${executionTime.toFixed(2)} ms, Memory: ${memoryUsed.toFixed(2)} MB`);
    ...
    test('should measure performance of NEAR operator', async () => {
      ...
      for (const size of testSizes) {
        ...
        for (const distance of distances) {
          ...
          // Log results
          console.log(`NEAR query with distance ${distance} (${contentSizeMB.toFixed(2)} MB): ${executionTime.toFixed(2)} ms, Memory: ${memoryUsed.toFixed(2)} MB`);
    ...
    test('should measure performance of fuzzy search', async () => {
      ...
      for (const size of testSizes) {
        ...
        for (const term of terms) {
          ...
          // Log results
          console.log(`Fuzzy search for "${term}" (${contentSizeMB.toFixed(2)} MB): ${executionTime.toFixed(2)} ms, Memory: ${memoryUsed.toFixed(2)} MB`);
