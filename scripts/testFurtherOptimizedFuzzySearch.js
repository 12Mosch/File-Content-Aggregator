...
const testCases = [
  ...
  },
  {
    ...
    content: fs.readFileSync(path.join(__dirname, "../src/electron/services/OptimizedFuzzySearchService.ts"), "utf8") +
             fs.readFileSync(path.join(__dirname, "../src/electron/services/NearOperatorService.ts"), "utf8"),
    term: "search",
    options: { isCaseSensitive: false }
  },
  {
...
async function runTests() {
  ...
  for (const testCase of testCases) {
    ...
    if (testCase.terms) {
      ...
      console.log(`Original implementation: ${originalTotalTime.toFixed(2)}ms (Matches: ${originalMatches}/${testCase.terms.length})`);
      console.log(`Optimized implementation: ${optimizedTotalTime.toFixed(2)}ms (Matches: ${optimizedMatches}/${testCase.terms.length})`);
      console.log(`Improvement: ${improvement.toFixed(2)}%`);
      console.log(`Match results are ${originalMatches === optimizedMatches ? "consistent" : "DIFFERENT"}`);
      ...
      // Print results for this test case
      console.log(`Original implementation: ${originalTime.toFixed(2)}ms (Matches: ${originalMatches}/${testCase.repeat})`);
      console.log(`Optimized implementation: ${optimizedTime.toFixed(2)}ms (Matches: ${optimizedMatches}/${testCase.repeat})`);
      console.log(`Improvement: ${improvement.toFixed(2)}%`);
      console.log(`Match results are ${(originalMatches > 0) === (optimizedMatches > 0) ? "consistent" : "DIFFERENT"}`);
    } else {
      // Standard test case
      ...
      // Print results for this test case
      console.log(`Original implementation: ${originalTime.toFixed(2)}ms (Match: ${originalResult.isMatch})`);
      console.log(`Optimized implementation: ${optimizedTime.toFixed(2)}ms (Match: ${optimizedResult.isMatch})`);
      console.log(`Improvement: ${improvement.toFixed(2)}%`);
      console.log(`Match results are ${originalResult.isMatch === optimizedResult.isMatch ? "consistent" : "DIFFERENT"}`);
    }
  }
  ...
  console.log("=================================================");
  console.log(`Original implementation total time: ${results.original.totalTime.toFixed(2)}ms`);
  console.log(`Optimized implementation total time: ${results.optimized.totalTime.toFixed(2)}ms`);
  
  const overallImprovement = (results.original.totalTime - results.optimized.totalTime) / results.original.totalTime * 100;
  ...
  console.log(`Optimized implementation matches: ${results.optimized.successCount}/${testCases.length}`);
  ...
  console.log("=================================================");
  console.log("Test Case | Original Time | Optimized Time | Improvement | Match Consistency");
  console.log("---------|--------------|---------------|------------|------------------");
  ...
  const resultsPath = path.join(__dirname, "../performance-results", `further-optimized-fuzzy-search-comparison-${timestamp}.json`);
