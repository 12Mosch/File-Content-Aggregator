...
const beforeMetrics = JSON.parse(
  fs.readFileSync(
    path.join(
      __dirname,
      "../performance-results/near-operator-metrics-2025-04-18T20-08-57.842Z.json"
    ),
    "utf8"
  )
);
...
const afterMetrics = JSON.parse(
  fs.readFileSync(
    path.join(
      __dirname,
      "../performance-results/near-operator-metrics-2025-04-18T20-19-52.486Z.json"
    ),
    "utf8"
  )
);
...
const timePercentage =
  (timeDiff / beforeMetrics.metrics.phaseMetrics.fuzzySearch.totalTime) * 100;
const avgTimeDiff =
...
const avgTimePercentage =
  (avgTimeDiff / beforeMetrics.metrics.phaseMetrics.fuzzySearch.averageTime) *
  100;
...
const analysisPath = path.join(
  __dirname,
  "../performance-results",
  "simplified-caching-analysis.txt"
);
fs.writeFileSync(
  analysisPath,
  `
