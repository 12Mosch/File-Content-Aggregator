...
async function runTests() {
  ...
  for (const testCase of testCases) {
    ...
    if (testCase.terms && !testCase.contents) {
      ...
      for (let r = 0; r < (testCase.repeat || 1); r++) {
        for (const term of testCase.terms) {
          service.search(testCase.content, term, testCase.options);
        }
      }
      ...
      for (let r = 0; r < (testCase.repeat || 1); r++) {
        for (const content of testCase.contents) {
          service.search(content, testCase.term, testCase.options);
        }
      }
      ...
      for (let i = 0; i < testCase.repeat; i++) {
        service.search(testCase.content, testCase.term, testCase.options);
      }
    }
    ...
    console.log("\nCache Statistics:");
    console.log(
      `- Result Cache: ${cacheStats.resultCache.hits} hits, ${cacheStats.resultCache.misses} misses, ${(cacheStats.resultCache.hitRate * 100).toFixed(2)}% hit rate`
    );
    console.log(
      `- Normalized String Cache: ${cacheStats.normalizedStringCache.hits} hits, ${cacheStats.normalizedStringCache.misses} misses, ${(cacheStats.normalizedStringCache.hitRate * 100).toFixed(2)}% hit rate`
    );
    console.log(
      `- Levenshtein Cache: ${cacheStats.levenshteinCache.hits} hits, ${cacheStats.levenshteinCache.misses} misses, ${(cacheStats.levenshteinCache.hitRate * 100).toFixed(2)}% hit rate`
    );
    console.log(
      `- RegExp Cache: ${cacheStats.regexCache.hits} hits, ${cacheStats.regexCache.misses} misses, ${(cacheStats.regexCache.hitRate * 100).toFixed(2)}% hit rate`
    );
  }
  ...
  console.log(
    `- Result Cache: ${comprehensiveCacheStats.resultCache.hits} hits, ${comprehensiveCacheStats.resultCache.misses} misses, ${(comprehensiveCacheStats.resultCache.hitRate * 100).toFixed(2)}% hit rate`
  );
  console.log(
    `- Normalized String Cache: ${comprehensiveCacheStats.normalizedStringCache.hits} hits, ${comprehensiveCacheStats.normalizedStringCache.misses} misses, ${(comprehensiveCacheStats.normalizedStringCache.hitRate * 100).toFixed(2)}% hit rate`
  );
  console.log(
    `- Levenshtein Cache: ${comprehensiveCacheStats.levenshteinCache.hits} hits, ${comprehensiveCacheStats.levenshteinCache.misses} misses, ${(comprehensiveCacheStats.levenshteinCache.hitRate * 100).toFixed(2)}% hit rate`
  );
  console.log(
    `- RegExp Cache: ${comprehensiveCacheStats.regexCache.hits} hits, ${comprehensiveCacheStats.regexCache.misses} misses, ${(comprehensiveCacheStats.regexCache.hitRate * 100).toFixed(2)}% hit rate`
  );

  // Print cache sizes
  console.log("\nCache Sizes:");
  console.log(
    `- Result Cache: ${comprehensiveCacheStats.resultCache.size} entries`
  );
  console.log(
    `- Normalized String Cache: ${comprehensiveCacheStats.normalizedStringCache.size} entries`
  );
  console.log(
    `- Levenshtein Cache: ${comprehensiveCacheStats.levenshteinCache.size} entries`
  );
  console.log(
    `- RegExp Cache: ${comprehensiveCacheStats.regexCache.size} entries`
  );
  console.log(
    `- Frequent Terms Cache: ${comprehensiveCacheStats.frequentTermsCache.size} entries`
  );
  console.log(
    `- Frequent Content Cache: ${comprehensiveCacheStats.frequentContentCache.size} entries`
  );
  console.log(
    `- Search Pattern Cache: ${comprehensiveCacheStats.searchPatternCache.size} entries`
  );
  ...
  const resultsPath = path.join(
    __dirname,
    "../performance-results",
    `cache-hit-rates-${timestamp}.json`
  );
  ...
  fs.writeFileSync(
    resultsPath,
    JSON.stringify(comprehensiveCacheStats, null, 2)
  );
