...
function processResults(results) {
  ...
  for (const result of results) {
    ...
    if (functions.length > 0) {
      ...
      const fileAvgComplexity = functions.reduce((sum, fn) => sum + fn.complexity, 0) / functions.length;
  ...
  files.sort((a, b) => parseFloat(b.averageComplexity) - parseFloat(a.averageComplexity));
...
function printSummary(data) {
  ...
  console.log(`High Complexity Functions (>= ${COMPLEXITY_THRESHOLD_WARNING}): ${summary.highComplexityFunctions}`);
  console.log('-'.repeat(80));
  ...
  for (let i = 0; i < top10Files.length; i++) {
    ...
    if (top3Functions.length > 0) {
      ...
      for (const fn of top3Functions) {
        console.log(`   - ${fn.name} (Line ${fn.line}): Complexity ${fn.complexity}`);
      }
    }
