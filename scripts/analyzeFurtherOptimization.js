...
const comparisonData = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, '../performance-results/further-optimized-fuzzy-search-comparison-2025-04-18T19-51-55.630Z.json'),
    'utf8'
  )
);
...
console.log(`Further optimized implementation total time: ${comparisonData.optimized.totalTime.toFixed(2)}ms`);
...
console.log(`Further optimized implementation matches: ${comparisonData.optimized.successCount}/${comparisonData.comparison.length}`);
...
const avgTimeImprovement = (beforeMetrics.metrics.phaseMetrics.fuzzySearch.averageTime - afterMetrics.metrics.phaseMetrics.fuzzySearch.averageTime) / beforeMetrics.metrics.phaseMetrics.fuzzySearch.averageTime * 100;
...
if (timeImprovement > 0) {
  console.log('✅ The further optimized fuzzy search implementation is faster overall in the NEAR operator.');
} else {
  console.log('❌ The further optimized fuzzy search implementation is not faster overall in the NEAR operator.');
}

if (avgTimeImprovement > 0) {
  console.log('✅ The average processing time per fuzzy search operation has improved.');
} else {
  console.log('❌ The average processing time per fuzzy search operation has not improved.');
}
...
if (timeImprovement > 10) {
  console.log('1. Adopt the further optimized fuzzy search implementation for all search operations.');
} else if (timeImprovement > 0) {
  console.log('1. Adopt the further optimized fuzzy search implementation with caution, monitoring performance in real-world usage.');
} else {
  console.log('1. Consider additional optimizations or a hybrid approach to improve performance further.');
}

console.log('2. Implement the string normalization caching for all case-insensitive operations.');
console.log('3. Implement the RegExp caching for all regular expression operations.');
console.log('4. Implement the Levenshtein distance caching for all fuzzy matching operations.');

// Save the analysis to a file
const analysisPath = path.join(__dirname, '../performance-results', 'further-fuzzy-search-optimization-analysis.txt');
