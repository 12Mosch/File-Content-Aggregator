...
try {
  ...
  if (!Array.isArray(beforeData) || !Array.isArray(afterData)) {
    console.error('Error: Input files must contain JSON arrays of test results');
    process.exit(1);
  }
  ...
  const allSizes = [...new Set([...Object.keys(groupedBefore), ...Object.keys(groupedAfter)])];
  ...
  for (const size of allSizes) {
    ...
    const execTimeImprovement = calculateImprovement(beforeAvgExecTime, afterAvgExecTime);
    const memoryImprovement = calculateImprovement(beforeAvgMemory, afterAvgMemory);
    
    console.log(`Execution Time: ${beforeAvgExecTime.toFixed(2)} ms → ${afterAvgExecTime.toFixed(2)} ms (${formatImprovement(execTimeImprovement)})`);
    console.log(`Memory Usage:   ${beforeAvgMemory.toFixed(2)} MB → ${afterAvgMemory.toFixed(2)} MB (${formatImprovement(memoryImprovement)})`);
  ...
  if (comparisonCount > 0) {
    ...
    console.log('-'.repeat(80));
    console.log(`Execution Time: ${formatImprovement(overallExecTimeImprovement)}`);
    console.log(`Memory Usage:   ${formatImprovement(overallMemoryImprovement)}`);
    console.log('='.repeat(80));
    ...
    console.log(`Actual Search Time Reduction: ${Math.abs(overallExecTimeImprovement).toFixed(2)}%`);
    console.log(`Status: ${Math.abs(overallExecTimeImprovement) >= 50 ? 'ACHIEVED ✓' : 'NOT YET ACHIEVED ✗'}`);
    console.log();
    console.log(`Memory Usage Reduction Goal: 30%`);
    console.log(`Actual Memory Usage Reduction: ${Math.abs(overallMemoryImprovement).toFixed(2)}%`);
    console.log(`Status: ${Math.abs(overallMemoryImprovement) >= 30 ? 'ACHIEVED ✓' : 'NOT YET ACHIEVED ✗'}`);
  }
