...
function generateReport(startTime, endTime, duration) {
  ...
  for (const file of resultFiles) {
    ...
    if (
      ...
      stats.mtime > latestResults[baseName].mtime
    ) {
      latestResults[baseName] = {
        file,
        mtime: stats.mtime,
...
function formatMemoryLeakResults(data) {
  ...
  if (data && data.summary) {
    ...
    if (data.iterations && data.iterations.length > 0) {
      ...
      const memoryValues = data.iterations.map((item) =>
        parseFloat(item.memoryChange)
      );
      ...
      for (let i = 0; i < data.iterations.length; i++) {
        ...
        const normalizedValue =
          range === 0 ? 0 : ((value - minValue) / range) * chartHeight;
