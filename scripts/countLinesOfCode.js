...
const REPORT_FILE = path.join(RESULTS_DIR, `code-metrics-${new Date().toISOString().replace(/:/g, '-')}.json`);
...
function countLines(filePath) {
  try {
    ...
    // Update by directory
    const directory = path.dirname(filePath).replace(SOURCE_DIR, '').replace(/^[\/\\]/, '') || 'root';
...
console.log(`Code Lines: ${stats.totalCodeLines} (${Math.round(stats.totalCodeLines / stats.totalLines * 100)}%)`);
console.log(`Comment Lines: ${stats.totalCommentLines} (${Math.round(stats.totalCommentLines / stats.totalLines * 100)}%)`);
console.log(`Blank Lines: ${stats.totalBlankLines} (${Math.round(stats.totalBlankLines / stats.totalLines * 100)}%)`);
console.log('-'.repeat(80));

console.log('\nTOP 5 FILE EXTENSIONS:');
console.log('-'.repeat(80));
Object.entries(stats.byExtension).slice(0, 5).forEach(([ext, data]) => {
  console.log(`${ext}: ${data.files} files, ${data.codeLines} code lines`);
});

console.log('\nTOP 5 DIRECTORIES:');
console.log('-'.repeat(80));
Object.entries(stats.byDirectory).slice(0, 5).forEach(([dir, data]) => {
