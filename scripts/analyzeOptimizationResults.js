...
const comparisonData = JSON.parse(
  fs.readFileSync(
    path.join(
      __dirname,
      "../performance-results/fuzzy-search-comparison-2025-04-18T19-36-47.614Z.json"
    ),
    "utf8"
  )
);
...
const beforeMetrics = JSON.parse(
  fs.readFileSync(
    path.join(
      __dirname,
      "../performance-results/near-operator-metrics-2025-04-18T19-25-23.824Z.json"
    ),
    "utf8"
  )
);
...
const countImprovement =
  ((beforeMetrics.metrics.phaseMetrics.fuzzySearch.count -
    afterMetrics.metrics.phaseMetrics.fuzzySearch.count) /
    beforeMetrics.metrics.phaseMetrics.fuzzySearch.count) *
  100;
const successRateImprovement =
  afterMetrics.metrics.phaseMetrics.fuzzySearch.successCount /
    afterMetrics.metrics.phaseMetrics.fuzzySearch.count -
  beforeMetrics.metrics.phaseMetrics.fuzzySearch.successCount /
    beforeMetrics.metrics.phaseMetrics.fuzzySearch.count;
...
if (timeImprovement > 0) {
  console.log(
    "✅ The optimized fuzzy search implementation is faster overall."
  );
} else {
  console.log(
    "❌ The optimized fuzzy search implementation is not faster overall."
  );
}

if (countImprovement > 0) {
  console.log(
    "✅ The optimized implementation reduces the number of fuzzy searches needed."
  );
} else {
  console.log(
    "❌ The optimized implementation does not reduce the number of fuzzy searches."
  );
}

if (successRateImprovement > 0) {
  console.log(
    "✅ The optimized implementation has a higher success rate for fuzzy searches."
  );
} else {
  console.log(
    "❌ The optimized implementation does not improve the success rate for fuzzy searches."
  );
}
...
const analysisPath = path.join(
  __dirname,
  "../performance-results",
  "fuzzy-search-optimization-analysis.txt"
);
fs.writeFileSync(
  analysisPath,
  `
