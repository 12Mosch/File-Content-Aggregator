...
export class PerformanceVisualizer {
   ...
   */
  public static generateOperationDurationChart(
   ...
   */
  public static generateOperationCallCountChart(
    summary: ProfileSummary
  ): ChartData {
    const topOps = summary.topOperations.slice(0, 10); // Limit to top 10 for readability
   ...
   */
  public static generateAverageDurationChart(
    summary: ProfileSummary
  ): ChartData {
    const topOps = summary.topOperations.slice(0, 10); // Limit to top 10 for readability
      ...
      datasets: [
        {
          ...
          backgroundColor: memoryOps.map((op) =>
            op.memoryDelta >= 0
              ? "rgba(255, 99, 132, 0.6)"
              : "rgba(75, 192, 192, 0.6)"
          ),
  ...
  private static transformReportToHierarchy(
    ...
    return {
      ...
      children: report.children.map((child) =>
        this.transformReportToHierarchy(child)
      ),
    };
  }
