...
export function CacheSettings() {
  ...
  useEffect(() => {
    ...
    const cacheId = Array.from(cacheInfo).find(c => c.name === selectedCache)?.name;
    if (!cacheId) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue={cacheInfo[0]?.name} value={selectedCache} onValueChange={setSelectedCache}>
        ...
        {cacheInfo.map(cache => (
          <TabsContent key={cache.name} value={cache.name} className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex justify-between items-center mt-1">
                      <span className="text-sm">{cache.size} / {cache.capacity}</span>
                      <Progress value={(cache.size / cache.capacity) * 100} className="h-2 w-32" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-center mt-1">
                      <span className="text-sm">{(cache.hitRate * 100).toFixed(1)}%</span>
                      <Progress value={cache.hitRate * 100} className="h-2 w-32" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm mt-1">{formatTime(cache.timeToLive)}</div>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Input
                      type="number"
                      value={maxSize}
                      onChange={(e) => setMaxSize(parseInt(e.target.value) || 100)}
                      className="w-20"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="enableTTL">{t('cache:enableExpiration')}</Label>
                    <Switch
                      id="enableTTL"
                      checked={enableTTL}
                  ...
                  {enableTTL && (
                    <div className="space-y-2 mt-2">
                      <Label htmlFor="timeToLive">{t('cache:timeToLive')}</Label>
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <Button
                          variant={timeToLive === 300000 ? "default" : "outline"}
                          onClick={() => setTimeToLive(300000)}
                        >
                          5 {t('cache:minutes')}
                        </Button>
                        <Button
                          variant={timeToLive === 1800000 ? "default" : "outline"}
                          onClick={() => setTimeToLive(1800000)}
                        >
                          30 {t('cache:minutes')}
                        </Button>
                        <Button
                          variant={timeToLive === 3600000 ? "default" : "outline"}
                          onClick={() => setTimeToLive(3600000)}
                        >
                          1 {t('cache:hour')}
                        </Button>
                        <Button
                          variant={timeToLive === 86400000 ? "default" : "outline"}
                          onClick={() => setTimeToLive(86400000)}
                        >
                          1 {t('cache:day')}
                        </Button>
                        <Button
                          variant={timeToLive === 604800000 ? "default" : "outline"}
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <Input
                          type="number"
                          value={timeToLive !== undefined ? Math.floor(timeToLive / 1000) : 60}
                          onChange={(e) => setTimeToLive(parseInt(e.target.value) * 1000 || 60000)}
                          className="w-20"
                        />
