...
export function SearchProgress({ searchId, onCancel, progress }: SearchProgressProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setElapsedTime(formatTime(performance.now() - (progress.startTime || performance.now())));
    }, 1000);
    
    return () => clearInterval(interval);
  }, [progress.elapsedTimeMs, progress.startTime]);
  ...
  return (
    <div className="bg-background border rounded-md p-4 mb-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-muted-foreground">{t('search:filesProcessed')}:</span>{' '}
        ...
        <div>
          <span className="text-muted-foreground">{t('search:matchesFound')}:</span>{' '}
          <span className="font-medium">{progress.matchesFound}</span>
        </div>
        
        <div>
          <span className="text-muted-foreground">{t('search:elapsedTime')}:</span>{' '}
          <span className="font-medium">{elapsedTime}</span>
        </div>
        
        {progress.currentFile && (
          <div className="col-span-2 truncate">
            <span className="text-muted-foreground">{t('search:currentFile')}:</span>{' '}
