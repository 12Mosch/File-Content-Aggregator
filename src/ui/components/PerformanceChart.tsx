...
const PerformanceChart: React.FC<PerformanceChartProps> = ({
  ...
  const drawBarChart = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    datasets.forEach((dataset, datasetIndex) => {
      const colors = dataset.backgroundColor instanceof Array 
        ? dataset.backgroundColor 
        : generateColors(barCount);
      
      dataset.data.forEach((value, index) => {
        const x = padding + index * (chartWidth / barCount) + datasetIndex * barWidth + barWidth / 2;
        const barHeight = (value / maxValue) * chartHeight;
        const y = height - padding - barHeight;
        
        ctx.fillStyle = Array.isArray(colors) ? colors[index] : (colors || generateColors(1)[0]);
        ...
        if (value > maxValue * 0.05) { // Only draw text if bar is tall enough
    ...
    labels.forEach((label, index) => {
      const x = padding + index * (chartWidth / barCount) + (chartWidth / barCount) / 2;
  ...
  const drawLineChart = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    if (datasets.length > 1) {
      ...
      datasets.forEach((dataset, index) => {
        const legendX = padding + index * 100;
        ctx.strokeStyle = dataset.borderColor || `hsl(${index * 137.5}, 70%, 50%)`;
        ctx.beginPath();
  ...
  const drawPieChart = (ctx: CanvasRenderingContext2D, width: number, height: number, isDoughnut: boolean = false) => {
    ...
    values.forEach((value, index) => {
      ...
      if (value / total > 0.05) {
        const labelRadius = (radius + innerRadius) / 2 + (innerRadius === 0 ? 0 : 10);
