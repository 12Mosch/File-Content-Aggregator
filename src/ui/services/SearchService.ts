...
export class SearchService {
  ...
  private processNextFile(
    ...
    this.workerPool.execute('search', {
      ...
      .then(response => {
        ...
        // Store result
        if (response.result) {
          state.results.set(currentFile.filePath, response.result);
          state.matchesFound += response.result.matchCount;
        }
   ...
   */
  public getFileResults(searchId: string, filePath: string): SearchResult | undefined {
    const searchState = this.activeSearches.get(searchId);
   ...
   * @param searchId The ID of the search
   */
  public getAllResults(searchId: string): Map<string, SearchResult> | undefined {
    const searchState = this.activeSearches.get(searchId);
    if (!searchState) return undefined;
