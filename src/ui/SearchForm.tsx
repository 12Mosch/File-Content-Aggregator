...
const SearchForm: React.FC<SearchFormProps> = ({
  ...
  const [formData, setFormData] = useState<SearchFormData>({
    searchPaths: "", extensions: "", excludeFiles: "",
    excludeFolders: ".git, node_modules, bin, obj, dist",
    folderExclusionMode: "contains", modifiedAfter: undefined,
    modifiedBefore: undefined, minSizeValue: "", minSizeUnit: "MB",
    maxSizeValue: "", maxSizeUnit: "MB", maxDepthValue: "",
  });
  ...
  useEffect(() => {
    if (historyEntryToLoad) {
      // ... (history loading logic remains the same) ...
      ...
      if (params.modifiedAfter) {
        try {
          ...
          else { const parsedISO = parseISO(params.modifiedAfter); if (isValid(parsedISO)) initialModifiedAfter = parsedISO; }
        } catch (e) { console.error("Error parsing modifiedAfter from history:", e); }
      }
      ...
      if (params.modifiedBefore) {
        try {
          ...
          else { const parsedISO = parseISO(params.modifiedBefore); if (isValid(parsedISO)) initialModifiedBefore = parsedISO; }
        } catch (e) { console.error("Error parsing modifiedBefore from history:", e); }
      }
      setFormData({
        ...
        folderExclusionMode: params.folderExclusionMode ?? "contains", modifiedAfter: initialModifiedAfter,
        modifiedBefore: initialModifiedBefore, minSizeValue: minSize.value, minSizeUnit: minSize.unit,
        maxSizeValue: maxSize.value, maxSizeUnit: maxSize.unit, maxDepthValue: params.maxDepth?.toString() ?? "",
      });
      setRawAfterDate(initialModifiedAfter ? format(initialModifiedAfter, DISPLAY_DATE_FORMAT, { locale: currentLocale }) : "");
      setRawBeforeDate(initialModifiedBefore ? format(initialModifiedBefore, DISPLAY_DATE_FORMAT, { locale: currentLocale }) : "");
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (e.target.type === "number" && parseFloat(value) < 0) {
      if (value === "" || parseFloat(value) === 0) setFormData((prev) => ({ ...prev, [name]: value }));
      return;
    }
  ...
  const parseDateString = (dateString: string): Date | undefined => {
    ...
    for (const fmt of PARSE_DATE_FORMATS) {
      try { const parsedDate = parse(dateString, fmt, new Date()); if (isValid(parsedDate)) return parsedDate; }
      catch (e) { /* ignore */ }
    } return undefined;
  };

  // Update raw state on input change
  const handleRawDateChange = (e: React.ChangeEvent<HTMLInputElement>, field: "modifiedAfter" | "modifiedBefore") => {
    const value = e.target.value;
    if (field === "modifiedAfter") setRawAfterDate(value);
    else setRawBeforeDate(value);

    // Attempt to parse and update the actual date state immediately
    const parsedDate = parseDateString(value);
     ...
     // --- REMOVED ---
     // Don't force popover open on every change
     // if (field === 'modifiedAfter' && !isAfterPopoverOpen) setIsAfterPopoverOpen(true);
     // if (field === 'modifiedBefore' && !isBeforePopoverOpen) setIsBeforePopoverOpen(true);
     // --- END REMOVED ---
  };

  // Process date on Enter or Escape
  const handleDateInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: "modifiedAfter" | "modifiedBefore") => {
    if (e.key === 'Enter') {
      e.preventDefault();
      ...
      if (parsedDate) {
        setFormData((prev) => ({ ...prev, [field]: parsedDate }));
        if (field === "modifiedAfter") setRawAfterDate(format(parsedDate, DISPLAY_DATE_FORMAT, { locale: currentLocale }));
        else setRawBeforeDate(format(parsedDate, DISPLAY_DATE_FORMAT, { locale: currentLocale }));
      } else if (!rawValue) {
         setFormData((prev) => ({ ...prev, [field]: undefined }));
      } else {
         const lastValidDate = formData[field];
         if (field === "modifiedAfter") setRawAfterDate(lastValidDate ? format(lastValidDate, DISPLAY_DATE_FORMAT, { locale: currentLocale }) : "");
         else setRawBeforeDate(lastValidDate ? format(lastValidDate, DISPLAY_DATE_FORMAT, { locale: currentLocale }) : "");
      }
  ...
  const handleDateSelect = (field: "modifiedAfter" | "modifiedBefore") => (date: Date | undefined) => {
    setFormData((prev) => ({ ...prev, [field]: date }));
    const formattedDate = date ? format(date, DISPLAY_DATE_FORMAT, { locale: currentLocale }) : "";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    handleDateInputKeyDown({ key: 'Enter', preventDefault: () => {} } as React.KeyboardEvent<HTMLInputElement>, 'modifiedAfter');
    handleDateInputKeyDown({ key: 'Enter', preventDefault: () => {} } as React.KeyboardEvent<HTMLInputElement>, 'modifiedBefore');
    setTimeout(() => {
      ...
      const submitParams: SearchParams = {
        searchPaths: splitAndClean(formData.searchPaths), extensions: splitAndClean(formData.extensions),
        excludeFiles: splitAndClean(formData.excludeFiles), excludeFolders: splitAndClean(formData.excludeFolders),
        folderExclusionMode: formData.folderExclusionMode, structuredQuery: queryStructure,
        contentSearchTerm: contentQueryString || undefined, contentSearchMode: contentQueryString ? "boolean" : undefined,
        caseSensitive: contentQueryString ? queryCaseSensitive : undefined,
        modifiedAfter: formData.modifiedAfter ? format(formData.modifiedAfter, "yyyy-MM-dd") : undefined,
        modifiedBefore: formData.modifiedBefore ? format(formData.modifiedBefore, "yyyy-MM-dd") : undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      {/* Date Fields Row - Revised Structure */}
                ...
                <div className="relative flex-grow">
                  ...
                  <CalendarIcon className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none" />
                   {formData.modifiedAfter && !isLoading && (
                     <Button variant="ghost" size="icon" type="button" onClick={(e) => { e.stopPropagation(); clearDate("modifiedAfter"); }} className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7 shrink-0 text-muted-foreground hover:text-destructive" aria-label="Clear modified after date">
                       <X className="h-4 w-4" />
                     </Button>
                   )}
                </div>
              ...
              <PopoverTrigger asChild>
                 <div className="relative flex-grow">
                   ...
                   {formData.modifiedBefore && !isLoading && (
                     <Button variant="ghost" size="icon" type="button" onClick={(e) => { e.stopPropagation(); clearDate("modifiedBefore"); }} className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7 shrink-0 text-muted-foreground hover:text-destructive" aria-label="Clear modified before date">
                       <X className="h-4 w-4" />
                     </Button>
                   )}
                 </div>
